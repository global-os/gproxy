...
export const globalPc = pgTable(
  ...
  (table) => [index('global_pc_user_id_idx').on(table.user_id)]
)

export const globalPcIcon = pgTable(
  'global_pc_icon',
  {
    id: serial('id').primaryKey(),
    global_pc_id: integer('global_pc_id')
      .notNull()
      .references(() => globalPc.id, { onDelete: 'cascade' }),
  ...
  (table) => [
    ...
    uniqueIndex('global_pc_icon_entry_uidx').on(
      table.global_pc_id,
      table.entry_name
    ),
  ]
...
export const workspace = pgTable(
  ...
  (table) => [index('workspace_global_pc_id_idx').on(table.global_pc_id)]
)
...
export const process = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex('process_workspace_directory_uidx').on(
      table.workspace_id,
      table.directory_id
    ),
  ]
...
export const task = pgTable(
  ...
  (table) => [index('task_global_pc_id_idx').on(table.global_pc_id)]
)
...
export const instances = pgTable(
  'instances',
  {
    ...
    process_id: integer('process_id').references(() => process.id, {
      onDelete: 'cascade',
    }),
  ...
  (table) => [index('instances_process_id_idx').on(table.process_id)]
)

/** LRU metadata for extracted instance bundles served from Postgres. */
export const instanceBundleCache = pgTable(
  'instance_bundle_cache',
  {
...
export const instanceBundleFile = pgTable(
  'instance_bundle_file',
  {
    instance_id: integer('instance_id')
      .notNull()
      .references(() => instances.id, { onDelete: 'cascade' }),
...
export const workspaceWindow = pgTable(
  'workspace_window',
  {
    id: serial('id').primaryKey(),
  ...
  (table) => [index('workspace_window_process_id_idx').on(table.process_id)]
)

export const workspaceLog = pgTable(
  'workspace_log',
  {
    id: serial('id').primaryKey(),
    workspace_id: integer('workspace_id')
  ...
  (table) => [index('workspace_log_workspace_id_idx').on(table.workspace_id)]
)
...
export const workspaceEvent = pgTable(
  ...
  (table) => [
    ...
    index('workspace_event_workspace_id_id_idx').on(
      table.workspace_id,
      table.id
    ),
  ]
...
export const webview = pgTable(
  ...
  (table) => [index('webview_process_id_idx').on(table.process_id)]
)

/** Per-webview routing rules (match + action). */
export const webviewRule = pgTable(
  'webview_rule',
  {
    id: serial('id').primaryKey(),
    webview_id: integer('webview_id')
      .notNull()
      .references(() => webview.id, { onDelete: 'cascade' }),
...
export const proxyTraffic = pgTable(
  'proxy_traffic',
  {
    id: serial('id').primaryKey(),
    session_id: integer('session_id')
      .notNull()
      .references(() => proxyRecordingSession.id, { onDelete: 'cascade' }),
