...
} from '../../../scripts/benchmark-scenarios.js'
...
const LOG_FORMAT = (
  flags['log-format'] === 'json' ? 'json' : 'human'
) as LogFormat
...
if (CONCURRENCY_RAW > MAX_CONCURRENCY) {
  logger.error(
    `Concurrency ${CONCURRENCY_RAW} exceeds maximum of ${MAX_CONCURRENCY}. Clamping to ${MAX_CONCURRENCY}.`
  )
}
if (CONCURRENCY_RAW > WARN_CONCURRENCY && CONCURRENCY_RAW <= MAX_CONCURRENCY) {
  warn(
    `Warning: concurrency ${CONCURRENCY_RAW} exceeds ${WARN_CONCURRENCY} — may hit sandbox rate limits.`
  )
}
...
function resolveApiKey(): string {
  ...
  logger.error(
    'Missing API key. Set ANTHROPIC_API_KEY or AI_GATEWAY_API_KEY (or store in macOS Keychain as ANTHROPIC_AUTH_TOKEN)'
  )
  process.exit(1)
}
...
async function uploadAndInstallPlugin(
  ...
  log(
    `  [plugin] Uploading ${entries.length} files to sandbox via writeFiles()...`
  )
  ...
  logger.event('plugin.upload.started', {
    file_count: fileBatch.length,
  ...
  for (let i = 0; i < fileBatch.length; i += CHUNK_SIZE) {
    ...
    try {
      ...
      logger.event('plugin.upload.chunk', {
        index: chunkIndex,
        total: totalChunks,
        bytes: chunkBytes,
      })
      ...
      logger.error(
        `  [plugin] writeFiles chunk ${i}-${i + chunk.length} failed: ${err.message}`
      )
...
async function loadCachedSnapshot(): Promise<string | null> {
  try {
    ...
    if (age < SNAPSHOT_MAX_AGE_MS) {
      log(
        `[snapshot] Using cached snapshot ${cache.snapshotId} (age: ${(age / 3600_000).toFixed(1)}h)`
      )
      return cache.snapshotId
    }
    log(
      `[snapshot] Cached snapshot expired (age: ${(age / 3600_000).toFixed(1)}h > 24h)`
    )
    return null
  } catch {
...
async function verifySnapshot(snapshotId: string): Promise<boolean> {
  ...
  try {
    ...
    if (ver.exit === 0 && ver.out.length > 0) {
      logger.event('snapshot.verify.succeeded', {
        snapshot_id: snapshotId,
        claude_version: ver.out.slice(0, 80),
      })
      return true
    }
    logger.event('snapshot.verify.failed', {
      snapshot_id: snapshotId,
      exit: ver.exit,
      stderr: ver.err.slice(0, 200),
    })
    return false
  } catch (err: any) {
    logger.event('snapshot.verify.failed', {
...
async function createSnapshot(): Promise<string> {
  ...
  const install = await run(sandbox, 'sh', [
    '-c',
...
async function runScenario(
  ...
  try {
    ...
    log(
      `  [${project.slug}] Sandbox ${sandbox.sandboxId} ready (${elapsed(t0)})`
    )

    // Install Claude Code (global npm packages don't survive snapshot restore)
    const claudeInstall = await run(sandbox, 'sh', [
      '-c',
      'npm install -g @anthropic-ai/claude-code',
    ])
    ...
    if (pluginInstall.out)
      log(
        `  [${project.slug}] add-plugin out: ${pluginInstall.out.slice(0, 200)}`
      )
    if (pluginInstall.err)
      log(
        `  [${project.slug}] add-plugin err: ${pluginInstall.err.slice(0, 200)}`
      )
    if (
      pluginInstall.exit !== 0 &&
      !pluginInstall.out.includes('successfully') &&
      !pluginInstall.out.includes('Installed')
    ) {
      throw new Error(
        `Plugin install failed (exit ${pluginInstall.exit}): ${(pluginInstall.out + ' ' + pluginInstall.err).slice(0, 300)}`
      )
    }
    ...
    await sandbox.writeFiles([
      { path: promptTmpPath, content: Buffer.from(project.prompt, 'utf-8') },
    ])
    ...
    if (claimDirs.length > 0) {
      const contents = await run(sandbox, 'sh', [
        '-c',
    ...
    if (seenFileCheck.out.length > 0) {
      const seenContents = await run(sandbox, 'cat', [
        seenFileCheck.out.split('\n')[0],
      ])
...
async function main() {
  ...
  logger.event('runner.start', {
    mode: flags.scenario ? 'single' : flags.quick ? 'quick' : 'full',
  })
  ...
  if (!flags['force-snapshot']) {
    ...
    if (snapshotId) {
      ...
      try {
        ...
        snapshotAgeHours =
          (Date.now() - new Date(cache.createdAt).getTime()) / 3600_000
      } catch {
        /* ignore */
      }
  ...
  if (!snapshotId) {
    ...
    logger.event('snapshot.create.succeeded', {
      snapshot_id: snapshotId,
      duration_ms: Math.round(snapshotCreationMs),
    })
  ...
  log(
    `Scenarios: ${scenarios.length} (${flags.quick ? 'quick' : (flags.scenario ?? 'full')})`
  )
  ...
  const snapshotMeta: SnapshotMeta = {
    ...
    ...(snapshotAgeHours !== undefined && {
      age_hours: Math.round(snapshotAgeHours * 10) / 10,
    }),
    ...(snapshotCreationMs !== undefined && {
      creation_duration_ms: Math.round(snapshotCreationMs),
    }),
  }

  const mode = flags.scenario
    ? ('single' as const)
    : flags.quick
      ? ('quick' as const)
      : ('full' as const)
  ...
  await writeFile(
    join(RESULTS_DIR, runId, 'run-manifest.json'),
  ...
  for (const s of summary.scenarios) {
    ...
    log(
      `${s.slug.padEnd(28)} ${s.status.toUpperCase().padEnd(10)} ${s.session_method.padEnd(12)} ${skills.padEnd(30)} ${dur}`
    )
  }
