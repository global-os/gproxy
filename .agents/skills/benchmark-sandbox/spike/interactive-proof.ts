...
type CmdResult = Awaited<ReturnType<InstanceType<typeof Sandbox>['runCommand']>>
...
async function main() {
  ...
  try {
    ...
    if (install.exit !== 0)
      throw new Error(
        `Claude Code install failed: ${install.err.slice(0, 300)}`
      )
    ...
    console.log(
      `  claim dirs: ${hasClaimDirs} (skills: ${claimedSkills.join(', ') || 'none'})`
    )
    console.log(
      `  seen-skills.txt: ${hasSeenFile} (${seenFileContents || 'empty'})`
    )
    console.log(`  debug logs: ${debugLogFiles.length} file(s)`)
    console.log(
      `  stderr traces: PreToolUse=${preToolUseInStderr} UserPromptSubmit=${userPromptInStderr}`
    )
    ...
    console.log(
      `  OK: ${Object.keys(artifacts).length} artifacts (${elapsed(t7)})`
    )
  ...
  if (sessionStep?.ok && hookStep?.ok) {
    ...
    console.log('Claude Code session completed but NO hook evidence found.')
    console.log(
      'Hooks may not fire in sandbox. Investigate PTY requirements or alternative session modes.'
    )
  } else {
    console.log('Claude Code session did NOT complete successfully.')
    console.log(
