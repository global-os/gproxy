...
const CACHE_ROOT = (() => {
  try {
    const installed = JSON.parse(
      readFileSync(
        resolve(process.env.HOME, '.claude/plugins/installed_plugins.json'),
        'utf-8'
      )
    )
    const entries =
      installed?.plugins?.['vercel-plugin@vercel-labs-vercel-plugin'] || []
...
const { loadSkills, matchSkills } = await import(
  join(PLUGIN_ROOT, 'hooks/pretooluse-skill-inject.mjs')
)
...
console.log(
  `🔧 Loaded ${Object.keys(skills.skillMap).length} skills from ${useCache ? 'CACHE' : 'DEV'}\n`
)
...
for (const entry of matchMatrix) {
  ...
  const shortTarget =
    entry.target.length > 70 ? entry.target.slice(0, 70) + '…' : entry.target
...
for (const entry of unmatched) {
  const shortTarget =
    entry.target.length > 80 ? entry.target.slice(0, 80) + '…' : entry.target
