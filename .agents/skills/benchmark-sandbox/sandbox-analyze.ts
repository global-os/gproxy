...
function parseHookCoverageFromDebugLogs(logContent: string): HookCoverage {
  return {
    sessionStart:
      logContent.includes('SessionStart') ||
      logContent.includes('session-start'),
    preToolUse:
...
async function analyzeRun(runDir: string): Promise<CoverageReport> {
  const manifest = await readJsonFile<RunSummary>(
  ...
  for (const s of manifest.scenarios) {
    ...
    if (s.hook_evidence.user_prompt_in_stderr)
      hookCoverage.userPromptSubmit = true
    ...
    const unexpectedSkills = s.claimed_skills.filter(
      (sk) => !expectedSet.has(sk)
    )
    const coveredCount = s.expected_skills.filter((sk) =>
      claimedSet.has(sk)
    ).length
    const scenarioCoverage =
      s.expected_skills.length > 0
        ? Math.round((coveredCount / s.expected_skills.length) * 100)
        : 100
  ...
  const allExpected = [
    ...new Set(manifest.scenarios.flatMap((s) => s.expected_skills)),
  ].sort()
  const allClaimed = [
    ...new Set(manifest.scenarios.flatMap((s) => s.claimed_skills)),
  ].sort()
  const scenariosWithExpectations = scenarios.filter(
    (s) => s.expectedSkills.length > 0
  )
  const coveragePercent =
    scenariosWithExpectations.length > 0
      ? Math.round(
          scenariosWithExpectations.reduce(
            (sum, s) => sum + s.coveragePercent,
            0
...
function printReport(report: CoverageReport): void {
  ...
  console.log(
    `Skill coverage: ${report.coveragePercent}% (${report.allClaimedSkills.length}/${report.allExpectedSkills.length} expected skills seen)`
  )
  console.log()
  ...
  console.log()
  console.log(
    'Hook legend: P=PreToolUse V=PostToolUse(validate) U=UserPromptSubmit S=SessionStart'
  )
  console.log()
  ...
  if (failures.length > 0) {
    ...
    for (const f of failures) {
      console.log(
        `  ${f.slug}: ${f.timedOut ? 'TIMEOUT' : (f.error ?? 'unknown error')}`
      )
      console.log(
        `    debug logs: ${f.debugLogLines} lines, stderr: ${f.stderrTraceLines} lines`
      )
    }
    console.log()
  }
