...
const getArg = (name: string, fallback: number) =>
  args.includes(`--${name}`)
    ? parseInt(args[args.indexOf(`--${name}`) + 1], 10)
    : fallback
...
if (SCENARIOS_FILE) {
  try {
    ...
    console.log(
      `Loaded ${ACTIVE_SCENARIOS.length} scenarios from ${SCENARIOS_FILE}`
    )
  } catch (e: any) {
    console.error(
      `Failed to load scenarios from ${SCENARIOS_FILE}: ${e.message}`
    )
    process.exit(1)
  }
}
...
async function collectPluginFiles(): Promise<
  Array<{ path: string; content: Buffer }>
> {
  const files: Array<{ path: string; content: Buffer }> = []
  async function walk(dir: string): Promise<void> {
    const entries = await readdir(join(LOCAL_PLUGIN_DIR, dir), {
      withFileTypes: true,
    })
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (entry.name.endsWith('.mts') || entry.name.endsWith('.test.ts'))
          continue
  ...
  for (const f of ['hooks/hooks.json', 'package.json']) {
    try {
      files.push({
        path: join(SANDBOX_PLUGIN_DIR, f),
        content: await readFile(join(LOCAL_PLUGIN_DIR, f)),
      })
    } catch {}
  }
  return files
...
export function calculateKeepAliveExtensionMs(
  keepAlive: boolean,
...
export function isExpectedRunCommandTimeout(error: unknown): boolean {
  ...
  return /timed out|deadline exceeded|operation timed out|request timeout/i.test(
    message
  )
}
...
export function evaluateBuildWaitState(input: BuildWaitInput): BuildWaitState {
  ...
  const sessionEnded = /\bSessionEnd\b/.test(
    normalizeShellOutput(input.debugLine)
  )
...
async function scoreWithHaiku(
  ...
  try {
    const sr = await sandbox.runCommand('sh', ['-c', cmd], {
...
const BUILD_SCORE_SCHEMA = {
  ...
  properties: {
    completeness: {
      type: 'string',
      enum: ['complete', 'partial', 'minimal', 'empty'],
    },
...
export function renderTimingMarkdownTable(
  results: Array<{ slug: string; timing?: ScenarioTiming }>
): string {
  let md = `## Timing\n\n`
...
async function readSandboxFileBuffer(
  sandbox: any,
  path: string
): Promise<Buffer | null> {
...
async function runScenario(
  ...
  try {
    ...
    try {
      appUrl = sandbox.domain(3000)
    } catch {}
    console.log(
      `  [${scenario.slug}] Sandbox ${sandbox.sandboxId}${appUrl ? ` | ${appUrl}` : ''} (${elapsed(t0)})`
    )
    ...
    console.log(
      `  [${scenario.slug}] claude=${claudeBin} agent-browser=${abBin} (${elapsed(t0)})`
    )
    ...
    await sandbox.runCommand('sh', [
      '-c',
      `mkdir -p ${projectDir} && cd ${projectDir} && npm init -y`,
    ])
    await sandbox.writeFiles(pluginFiles)
    await sh(
      sandbox,
      `cd ${projectDir} && npx -y add-plugin ${SANDBOX_PLUGIN_DIR} -s project -y --target claude-code 2>&1 | tail -1`
    )
    ...
    await sandbox.writeFiles([
      { path: '/tmp/prompt.txt', content: Buffer.from(scenario.prompt) },
    ])
    ...
    async function pollBuildProgress(
      ...
      try {
        const [
          ...
          sh(
            sandbox!,
            `find ${projectDir} -maxdepth 3 -not -path '*/node_modules/*' -not -path '*/.git/*' -not -path '*/.next/*' -not -path '*/.claude/*' -newer /tmp/prompt.txt -type f 2>/dev/null | wc -l`
          ),
          sh(
            sandbox!,
          ...
          includeProcessState
            ? sh(
                sandbox!,
                'ls -t ~/.claude/debug/*.txt 2>/dev/null | head -1 | xargs tail -1 2>/dev/null || true'
              )
            : Promise.resolve(''),
        ])
        const skills = normalizeShellOutput(skillsRaw)
          .split('\n')
          .filter(Boolean)
        ...
        const waitState = includeProcessState
          ...
          : null
        const staleSuffix = waitState?.stale
          ? ` | STALE(${Math.round(msSinceLastDebugChange / 1000)}s)`
          : ''
        const claudeStatus = waitState
          ? waitState.claudeRunning
            ? 'running'
            : `exited(${normalizeShellOutput(claudeProbeRaw)})`
          : ''
        const waitSuffix = waitState
          ...
          : ''
        console.log(
          `  [${scenario.slug}] ${elapsed(t0)} | skills: ${skills.join(', ') || '(none)'} | files: ${fileCount} | :3000=${port3000}${waitSuffix}`
        )
        return waitState
      } catch {
        return null
    ...
    async function readClaudeExitCode(
      exitPath: string,
      logPath: string
    ): Promise<number | null> {
      for (let attempt = 1; attempt <= 3; attempt++) {
        const exitRaw = await sh(
          sandbox!,
          `cat ${exitPath} 2>/dev/null || grep -o 'EXIT:[0-9]\\+' ${logPath} 2>/dev/null | tail -1`
        )
    ...
    console.log(
      `  [${scenario.slug}] Phase 1: BUILD started (single launch + fire-and-forget poll after API timeout) (${elapsed(t0)})`
    )
    ...
    try {
      ...
      if (isExpectedRunCommandTimeout(e)) {
        ...
        buildExit = 124
        console.log(
          `  [${scenario.slug}] Build launch hit the expected 300s API timeout; waiting on sandbox process state (${elapsed(t0)})`
        )
      } else {
        console.log(
          `  [${scenario.slug}] Build launch failed | attempt=runCommand | state=launching | error=${e.message?.slice(0, 200)} (${elapsed(t0)})`
        )
        buildExit = 1
      }
    } finally {
      ...
      while (true) {
        ...
        if (!waitState) {
          if (Date.now() - buildStartedAt >= TIMEOUT_MS) {
            buildExit = 124
            console.log(
              `  [${scenario.slug}] Build wait probe stalled past phase timeout (${TIMEOUT_MS / 1000}s) (${elapsed(t0)})`
            )
            break
          }
          console.log(
            `  [${scenario.slug}] Build wait probe failed; retrying in ${BUILD_WAIT_POLL_MS / 1000}s (${elapsed(t0)})`
          )
          await new Promise((resolve) =>
            setTimeout(resolve, BUILD_WAIT_POLL_MS)
          )
          continue
        }
        if (!waitState.shouldKeepPolling) {
          if (waitState.timedOut) {
            buildExit = 124
            console.log(
              `  [${scenario.slug}] Build wait reached phase timeout (${TIMEOUT_MS / 1000}s) (${elapsed(t0)})`
            )
          } else if (waitState.stale) {
            const exitCode = await readClaudeExitCode(exitFile, logFile)
            if (exitCode !== null) buildExit = exitCode
            console.log(
              `  [${scenario.slug}] Build stale — no new debug log activity for ${BUILD_STALE_THRESHOLD_MS / 1000}s, treating as done | exit=${exitCode ?? 'unknown'} (${elapsed(t0)})`
            )
          } else {
            const exitCode = await readClaudeExitCode(exitFile, logFile)
            if (exitCode !== null) buildExit = exitCode
            console.log(
              `  [${scenario.slug}] Build process finished | claude=${waitState.claudeRunning ? 'running' : 'exited'} | sessionEnd=${waitState.sessionEnded ? 'yes' : 'no'} | exit=${exitCode ?? 'unknown'} (${elapsed(t0)})`
            )
          }
          break
        }
    ...
    try {
      ...
      if (tail)
        console.log(
          `  [${scenario.slug}] Build stdout (last 30 lines):\n${tail}`
        )
      if (errTail)
        console.log(
          `  [${scenario.slug}] Build stderr (last 10 lines):\n${errTail}`
        )
    } catch {}
    ...
    const projectFilesList = (
      await sh(
        sandbox,
        `find ${projectDir} -maxdepth 3 -not -path '*/node_modules/*' -not -path '*/.git/*' -not -path '*/.next/*' -not -path '*/.claude/*' -type f 2>/dev/null | head -40`
      )
    )
      .split('\n')
      .filter(Boolean)
    ...
    if (projectFilesList.length > 1) {
      try {
        console.log(
          `  [${scenario.slug}] Snapshotting after build... (${elapsed(t0)})`
        )
        try {
          await sandbox.extendTimeout(SNAPSHOT_PREP_EXTENSION_MS)
        } catch {}
        const snap = await sandbox.snapshot({ expiration: 4 * 3600_000 }) // 4 hour expiry
        snapshotId = snap.snapshotId
        console.log(
          `  [${scenario.slug}] Snapshot created: ${snapshotId} (${elapsed(t0)})`
        )
        ...
        console.log(
          `  [${scenario.slug}] Restored from snapshot: ${sandbox.sandboxId}${appUrl ? ` | ${appUrl}` : ''} (${elapsed(t0)})`
        )
      } catch (e: any) {
        console.log(
          `  [${scenario.slug}] Snapshot failed (continuing with original sandbox): ${e.message?.slice(0, 100)} (${elapsed(t0)})`
        )
      }
    }

    const claimedSkills = (
      await sh(sandbox, 'ls /tmp/vercel-plugin-*-seen-skills.d/ 2>/dev/null')
    )
      .split('\n')
      .filter(Boolean)
    console.log(
      `  [${scenario.slug}] Build done (exit=${buildExit}) | skills=${claimedSkills.length} | files=${projectFilesList.length} (${elapsed(t0)})`
    )

    // Score build completeness with haiku
    let buildScore: HaikuScore | null = null
    if (projectFilesList.length > 0) {
      const fileList = projectFilesList
        .map((f) => f.replace(`${SANDBOX_HOME}/${scenario.slug}/`, ''))
        .join('\n')
      buildScore = await scoreWithHaiku(
        sandbox,
        claudeBin,
      ...
      if (buildScore) {
        console.log(
          `  [${scenario.slug}] Build score: ${(buildScore as any).completeness} | API=${(buildScore as any).hasApiRoutes} UI=${(buildScore as any).hasUIComponents} AI=${(buildScore as any).hasAIFeature} (${elapsed(t0)})`
        )
      }
    }
    ...
    if (portCheck === '200' || portCheck === '307') {
      port3000Up = true
      console.log(
        `  [${scenario.slug}] Dev server already running (${elapsed(t0)})`
      )
    } else {
      const hasNext = await sh(
        sandbox,
        `test -f ${projectDir}/node_modules/.bin/next && echo YES || echo NO`
      )
      if (hasNext === 'YES') {
        console.log(
          `  [${scenario.slug}] Starting dev server... (${elapsed(t0)})`
        )
        await sh(
          sandbox,
          `cd ${projectDir} && nohup npx next dev --port 3000 --turbopack > /tmp/next-dev.log 2>&1 & echo started`
        )
        for (let i = 0; i < 30; i++) {
          ...
          if (status === '200' || status === '307') {
            ...
            console.log(
              `  [${scenario.slug}] Dev server UP: ${appUrl} (${elapsed(t0)})`
            )
            break
          }
        }
    ...
    const keepAliveExtensionMs = calculateKeepAliveExtensionMs(
      KEEP_ALIVE,
      KEEP_ALIVE_HOURS
    )
    if (keepAliveExtensionMs) {
      try {
        ...
        console.log(
          `  [${scenario.slug}] Keep-alive timeout extended (${elapsed(t0)})`
        )
      } catch (e: any) {
        console.log(
          `  [${scenario.slug}] extendTimeout: ${e.message?.slice(0, 60)}`
        )
      }
    }
    ...
    const vercelProjectName =
      `${scenario.slug}-${new Date().toISOString().replace(/[-:T]/g, '').slice(0, 12)}`.toLowerCase()
    if (vercelToken) {
      try {
        const linkOut = await sh(
          sandbox,
          `cd ${projectDir} && unset VERCEL_TOKEN && vercel link --yes --scope vercel-labs --project ${vercelProjectName} 2>&1 | tail -3`
        )
        console.log(
          `  [${scenario.slug}] Linked to vercel-labs/${vercelProjectName}: ${linkOut.split('\n').pop()} (${elapsed(t0)})`
        )
        const envOut = await sh(
          sandbox,
          `cd ${projectDir} && unset VERCEL_TOKEN && vercel env pull .env.local --yes 2>&1 | tail -3`
        )
        console.log(
          `  [${scenario.slug}] Pulled env: ${envOut.split('\n').pop()} (${elapsed(t0)})`
        )
      } catch (e: any) {
        console.log(
          `  [${scenario.slug}] Vercel link/env pull failed: ${e.message?.slice(0, 80)} (${elapsed(t0)})`
        )
      }
    }
    ...
    try {
      const existing = await sh(
        sandbox,
        `cat ${projectDir}/.env.local 2>/dev/null`
      )
      const appBaseUrl = baseUrl.endsWith('/v1') ? baseUrl : `${baseUrl}/v1`
      if (
        !existing.includes('ANTHROPIC_BASE_URL') ||
        !existing.includes('/v1')
      ) {
        ...
        await sandbox.writeFiles([
          {
            ...
            content: Buffer.from(envWithBaseUrl),
          },
        ])
        console.log(
          `  [${scenario.slug}] Ensured ANTHROPIC_BASE_URL=${appBaseUrl} in .env.local (${elapsed(t0)})`
        )
      }
    } catch {}
    ...
    if (!SKIP_VERIFY && projectFilesList.length > 1) {
      console.log(
        `  [${scenario.slug}] Phase 2: VERIFY with agent-browser (${elapsed(t0)})`
      )
      timing.verifyStartMs = captureRelativeMs(t0)
      const verifyPrompt = buildVerificationPrompt(scenario.userStories)
      await sandbox.writeFiles([
        { path: '/tmp/verify.txt', content: Buffer.from(verifyPrompt) },
      ])
      ...
      try {
        ...
        if (isExpectedRunCommandTimeout(e)) {
          console.log(
            `  [${scenario.slug}] Verify hit 300s API timeout; polling for completion (${elapsed(t0)})`
          )
          // Poll for verify exit
          const verifyStartedAt = Date.now()
          while (Date.now() - verifyStartedAt < 1_200_000) {
            // 20 min max
            await new Promise((r) => setTimeout(r, BUILD_WAIT_POLL_MS))
            const exitRaw = await sh(
              sandbox,
              `cat ${verifyExitFile} 2>/dev/null || echo RUNNING`
            )
            const probe = normalizeShellOutput(exitRaw)
            if (probe !== 'RUNNING' && probe !== '') {
              verifyExit = parseInt(probe, 10) || 0
              console.log(
                `  [${scenario.slug}] Verify exited(${verifyExit}) (${elapsed(t0)})`
              )
              break
            }
            // Log progress
            const tail = await sh(
              sandbox,
              `tail -1 ${verifyLogFile} 2>/dev/null`
            )
            console.log(
              `  [${scenario.slug}] Verify polling... | ${elapsed(t0)} | ${normalizeShellOutput(tail).slice(-100)}`
            )
          }
          if (verifyExit === -1) {
            verifyExit = 124
            console.log(
              `  [${scenario.slug}] Verify timed out after 20min (${elapsed(t0)})`
            )
          }
        } else {
          verifyExit = 1
          console.log(
            `  [${scenario.slug}] Verify launch failed: ${e.message?.slice(0, 100)} (${elapsed(t0)})`
          )
        }
      } finally {
      ...
      try {
        ...
        console.log(
          `  [${scenario.slug}] Verify stdout: ${outLines.length} lines, last: ${outLines.slice(-3).join(' | ').slice(-200)}`
        )
        if (normalizeShellOutput(verifyErr)) {
          console.log(
            `  [${scenario.slug}] Verify stderr (last 3 lines): ${normalizeShellOutput(verifyErr).split('\n').slice(-3).join(' | ').slice(-200)}`
          )
        }
      } catch {}

      // Re-extract skills after verify phase (agent-browser + fixes trigger more)
      const postVerifySkills = (
        await sh(sandbox, 'ls /tmp/vercel-plugin-*-seen-skills.d/ 2>/dev/null')
      )
        .split('\n')
        .filter(Boolean)
      if (postVerifySkills.length > claimedSkills.length) {
        const newSkills = postVerifySkills.filter(
          (s) => !claimedSkills.includes(s)
        )
        if (newSkills.length > 0) {
          console.log(
            `  [${scenario.slug}] +${newSkills.length} skills from verify: ${newSkills.join(', ')}`
          )
      ...
      const storyList = scenario.userStories
        .map((s, i) => `${i + 1}. ${s}`)
        .join('\n')
      const verifyScore = await scoreWithHaiku(
        sandbox,
        claudeBin,
      ...
      let stories: VerificationResult['stories'] = scenario.userStories.map(
        (_, i) => ({
          index: i + 1,
      ...
      verification = {
        ...
        exitCode: verifyExit,
        stories,
        output: verifyOut.slice(-500),
      }
      const passCount = stories.filter((s) => s.status === 'pass').length
      console.log(
        `  [${scenario.slug}] Verify: ${passCount}/${stories.length} passed (exit=${verifyExit}) (${elapsed(t0)})`
      )
    } else if (SKIP_VERIFY) {
      console.log(`  [${scenario.slug}] Verification skipped (--skip-verify)`)
    } else {
      console.log(
        `  [${scenario.slug}] Verification skipped (only ${projectFilesList.length} files built)`
      )
    }
...
- Deployment protection is enabled by default, which is what we want`
      ...
      await sandbox.writeFiles([
        { path: '/tmp/deploy.txt', content: Buffer.from(deployPrompt) },
      ])
      ...
      async function pollDeployProgress(): Promise<BuildWaitState | null> {
        try {
          ...
          const staleSuffix = waitState.stale
            ? ` | STALE(${Math.round(msSinceLastDebugChange / 1000)}s)`
            : ''
          const claudeStatus = waitState.claudeRunning
            ? 'running'
            : `exited(${normalizeShellOutput(claudeProbeRaw)})`
          console.log(
            `  [${scenario.slug}] Deploy wait | claude=${claudeStatus} | sessionEnd=${waitState.sessionEnded ? 'yes' : 'no'}${waitState.timedOut ? ' | wait=timeout' : ''}${staleSuffix}${debugLine ? ` | debug=${debugLine.slice(-120)}` : ''} (${elapsed(t0)})`
          )
          return waitState
        } catch {
          return null
      ...
      try {
        ...
        if (isExpectedRunCommandTimeout(e)) {
          ...
          deployExit = 124
          console.log(
            `  [${scenario.slug}] Deploy launch hit the expected 300s API timeout; waiting on sandbox process state (${elapsed(t0)})`
          )
        } else {
          deployExit = 1
          console.log(
            `  [${scenario.slug}] Deploy launch failed | attempt=runCommand | state=launching | error=${e.message?.slice(0, 200)} (${elapsed(t0)})`
          )
        }
      }
        ...
        while (true) {
          ...
          if (!waitState) {
            if (Date.now() - deployStartedAt >= TIMEOUT_MS) {
              deployExit = 124
              console.log(
                `  [${scenario.slug}] Deploy wait probe stalled past phase timeout (${TIMEOUT_MS / 1000}s) (${elapsed(t0)})`
              )
              break
            }
            console.log(
              `  [${scenario.slug}] Deploy wait probe failed; retrying in ${BUILD_WAIT_POLL_MS / 1000}s (${elapsed(t0)})`
            )
            await new Promise((resolve) =>
              setTimeout(resolve, BUILD_WAIT_POLL_MS)
            )
          ...
          if (!waitState.shouldKeepPolling) {
            if (waitState.timedOut) {
              deployExit = 124
              console.log(
                `  [${scenario.slug}] Deploy wait reached phase timeout (${TIMEOUT_MS / 1000}s) (${elapsed(t0)})`
              )
            } else if (waitState.stale) {
              const exitCode = await readClaudeExitCode(
                deployExitFile,
                deployLogFile
              )
              if (exitCode !== null) deployExit = exitCode
              console.log(
                `  [${scenario.slug}] Deploy stale — no new debug log activity for ${BUILD_STALE_THRESHOLD_MS / 1000}s, treating as done | exit=${exitCode ?? 'unknown'} (${elapsed(t0)})`
              )
            } else {
              const exitCode = await readClaudeExitCode(
                deployExitFile,
              ...
              if (exitCode !== null) deployExit = exitCode
              console.log(
                `  [${scenario.slug}] Deploy process finished | claude=${waitState.claudeRunning ? 'running' : 'exited'} | sessionEnd=${waitState.sessionEnded ? 'yes' : 'no'} | exit=${exitCode ?? 'unknown'} (${elapsed(t0)})`
              )
            }
            break
          }
      ...
      if (urlMatch) {
        ...
        console.log(
          `  [${scenario.slug}] Deployed: ${deployUrl} (${elapsed(t0)})`
        )
      }

      // Re-extract skills after deploy phase
      const postDeploySkills = (
        await sh(sandbox, 'ls /tmp/vercel-plugin-*-seen-skills.d/ 2>/dev/null')
      )
        .split('\n')
        .filter(Boolean)
      if (postDeploySkills.length > claimedSkills.length) {
        const newSkills = postDeploySkills.filter(
          (s) => !claimedSkills.includes(s)
        )
        if (newSkills.length > 0) {
          console.log(
            `  [${scenario.slug}] +${newSkills.length} skills from deploy: ${newSkills.join(', ')}`
          )
          claimedSkills.push(...newSkills)
        }
      }
      ...
      deployScore = await scoreWithHaiku(
        sandbox,
        claudeBin,
      ...
      if (deployScore) {
        ...
        console.log(
          `  [${scenario.slug}] Deploy score: deployed=${ds.deployed} build=${ds.buildSucceeded}${ds.url ? ` url=${ds.url}` : ''} (${elapsed(t0)})`
        )
        // If haiku found a URL we missed, use it
        if (!deployUrl && ds.url && ds.url.includes('vercel.app')) {
          deployUrl = ds.url
          console.log(
            `  [${scenario.slug}] Deploy URL from haiku: ${deployUrl}`
          )
        }
      }
      ...
      if (!deployUrl) {
        console.log(
          `  [${scenario.slug}] Deploy failed (exit=${deployExit}) (${elapsed(t0)})`
        )
      }
    }
    ...
    const trackArtifact = async (
      ...
      console.log(
        `  [${scenario.slug}] Artifact saved | file=${fileName} | sizeKB=${(artifact.sizeBytes / 1024).toFixed(0)} (${elapsed(t0)})`
      )
      return artifact
    }
    ...
    try {
      await sh(
        sandbox,
        `tar czf /tmp/debug-logs.tar.gz -C ${SANDBOX_HOME} .claude/debug/ 2>/dev/null`
      )
      const debugBuffer = await readSandboxFileBuffer(
        sandbox,
        '/tmp/debug-logs.tar.gz'
      )
      if (!debugBuffer)
        throw new Error('sandbox file /tmp/debug-logs.tar.gz missing')
      await trackArtifact(
        'debug-logs.tar.gz',
        'Full Claude debug logs captured from ~/.claude/debug.',
        debugBuffer
      )
    } catch (e: any) {
      console.log(
        `  [${scenario.slug}] Artifact extract failed | artifact=debug-logs.tar.gz | attempted=tar-debug-logs | error=${e.message?.slice(0, 160)}`
      )
    }
    ...
    try {
      await sh(
        sandbox,
        'tar czf /tmp/claude-output.tar.gz /tmp/claude-build-round-*.log /tmp/claude-build-round-*.err /tmp/claude-verify.log /tmp/claude-verify.err /tmp/claude-deploy.log /tmp/claude-deploy.err 2>/dev/null'
      )
      const claudeOutputBuffer = await readSandboxFileBuffer(
        sandbox,
        '/tmp/claude-output.tar.gz'
      )
      if (!claudeOutputBuffer)
        throw new Error('sandbox file /tmp/claude-output.tar.gz missing')
      await trackArtifact(
        'claude-output.tar.gz',
        'Claude build/verify/deploy stdout/stderr logs.',
        claudeOutputBuffer
      )
    } catch (e: any) {
      console.log(
        `  [${scenario.slug}] Artifact extract failed | artifact=claude-output.tar.gz | attempted=tar-claude-output | error=${e.message?.slice(0, 160)}`
      )
    }
    ...
    try {
      await sh(
        sandbox,
        `cd ${SANDBOX_HOME} && tar czf /tmp/source.tar.gz --exclude='node_modules' --exclude='.next' --exclude='.git' ${scenario.slug}/ 2>/dev/null`
      )
      const sourceBuffer = await readSandboxFileBuffer(
        sandbox,
        '/tmp/source.tar.gz'
      )
      if (!sourceBuffer)
        throw new Error('sandbox file /tmp/source.tar.gz missing')
      const artifact = await trackArtifact(
        'source.tar.gz',
        'Generated project source tree excluding node_modules, .next, and .git.',
        sourceBuffer
      )
      sourcePath = join(archiveDir, artifact.fileName)
    } catch (e: any) {
      console.log(
        `  [${scenario.slug}] Artifact extract failed | artifact=source.tar.gz | attempted=tar-project-source | error=${e.message?.slice(0, 160)}`
      )
    }
    ...
    try {
      ...
      const hookTraceOutput =
        normalizeShellOutput(hookTrace) ||
        'No hook/session trace lines matched.'
      await trackArtifact(
        'hook-trace.txt',
        'Filtered hook and session trace lines from Claude debug logs.',
        `${hookTraceOutput}\n`
      )
    } catch (e: any) {
      console.log(
        `  [${scenario.slug}] Artifact extract failed | artifact=hook-trace.txt | attempted=grep-hook-trace | error=${e.message?.slice(0, 160)}`
      )
    }
    ...
    try {
      ...
      const fileTreeOutput =
        normalizeShellOutput(fileTree) || 'No project files found.'
      await trackArtifact(
        'file-tree.txt',
        'Readable file tree for the generated project.',
        `${fileTreeOutput}\n`
      )
    } catch (e: any) {
      console.log(
        `  [${scenario.slug}] Artifact extract failed | artifact=file-tree.txt | attempted=find-project-files | error=${e.message?.slice(0, 160)}`
      )
    }

    try {
      const pluginEnv = await sh(sandbox, 'env | grep -i vercel_plugin || true')
      const pluginEnvOutput =
        normalizeShellOutput(pluginEnv) ||
        'No VERCEL_PLUGIN environment variables found.'
      await trackArtifact(
        'plugin-env.txt',
        'Sandbox environment variables matching VERCEL_PLUGIN.',
        `${pluginEnvOutput}\n`
      )
    } catch (e: any) {
      console.log(
        `  [${scenario.slug}] Artifact extract failed | artifact=plugin-env.txt | attempted=grep-plugin-env | error=${e.message?.slice(0, 160)}`
      )
    }

    try {
      const npmGlobals = await sh(
        sandbox,
        'npm ls -g --depth=0 2>/dev/null || true'
      )
      const npmGlobalsOutput =
        normalizeShellOutput(npmGlobals) ||
        'npm global package listing returned no output.'
      await trackArtifact(
        'npm-globals.txt',
        'Globally installed npm packages inside the sandbox.',
        `${npmGlobalsOutput}\n`
      )
    } catch (e: any) {
      console.log(
        `  [${scenario.slug}] Artifact extract failed | artifact=npm-globals.txt | attempted=npm-ls-global | error=${e.message?.slice(0, 160)}`
      )
    }

    try {
      const settingsExists = await sh(
        sandbox,
        `test -f ${settingsPath} && echo present || true`
      )
      if (normalizeShellOutput(settingsExists) === 'present') {
        const settingsBuffer = await readSandboxFileBuffer(
          sandbox,
          settingsPath
        )
        if (!settingsBuffer)
          throw new Error(`sandbox file ${settingsPath} missing`)
        await trackArtifact(
          'settings.json',
          'Claude settings generated for the scenario project.',
          settingsBuffer
        )
      }
    } catch (e: any) {
      console.log(
        `  [${scenario.slug}] Artifact extract failed | artifact=settings.json | attempted=copy-settings | error=${e.message?.slice(0, 160)}`
      )
    }

    try {
      const toolCallsRaw = await sh(
        sandbox,
        "grep -Rni 'executePreToolHooks called' ~/.claude/debug 2>/dev/null || true"
      )
      await trackArtifact(
        'tool-calls.txt',
        'Pre-tool hook calls with parsed timestamp and tool name.',
        `${summarizeToolCalls(toolCallsRaw)}\n`
      )
    } catch (e: any) {
      console.log(
        `  [${scenario.slug}] Artifact extract failed | artifact=tool-calls.txt | attempted=grep-pretool-hooks | error=${e.message?.slice(0, 160)}`
      )
    }

    try {
      const claimDirs = await sh(
        sandbox,
        'ls -la /tmp/vercel-plugin-*-seen-skills.d/ 2>/dev/null || true'
      )
      const claimFiles = await sh(
        sandbox,
        'cat /tmp/vercel-plugin-*-seen-skills.txt 2>/dev/null || true'
      )
      const claimsOutput = [
        '=== claim directories ===',
      ...
      await trackArtifact(
        'skill-claims.txt',
        'Seen-skills claim directories and files emitted by the plugin.',
        `${claimsOutput}\n`
      )
    } catch (e: any) {
      console.log(
        `  [${scenario.slug}] Artifact extract failed | artifact=skill-claims.txt | attempted=collect-skill-claims | error=${e.message?.slice(0, 160)}`
      )
    }
    ...
    try {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        await writeFile(
          manifestPath,
          JSON.stringify(buildArtifactManifest(manifestEntries), null, 2)
        )
      ...
      console.log(
        `  [${scenario.slug}] Artifact saved | file=manifest.json | sizeKB=${(manifestEntry.sizeBytes / 1024).toFixed(0)} (${elapsed(t0)})`
      )
    } catch (e: any) {
      console.log(
        `  [${scenario.slug}] Artifact extract failed | artifact=manifest.json | attempted=write-manifest | error=${e.message?.slice(0, 160)}`
      )
    }
    ...
    console.log(
      `  [${scenario.slug}] DONE (${elapsed(t0)}) | skills=${claimedSkills.length} | files=${projectFilesList.length}${deployUrl ? ` | ${deployUrl}` : appUrl ? ` | ${appUrl}` : ''}`
    )
    ...
    if (snapshotId) {
      try {
        ...
        await snap.delete()
        console.log(
          `  [${scenario.slug}] Snapshot ${snapshotId} cleaned up (${elapsed(t0)})`
        )
      } catch {}
    }
...
async function generateReport(
  ...
  const scenarioMap = Object.fromEntries(
    ACTIVE_SCENARIOS.map((s) => [s.slug, s])
  )
  const totalSkills = new Set(results.flatMap((r) => r.claimedSkills))
  const verified = results.filter((r) => r.verification?.ran)
  const totalStories = verified.reduce(
    (a, r) => a + r.verification!.stories.length,
    0
  )
  const passedStories = verified.reduce(
    (a, r) =>
      a + r.verification!.stories.filter((s) => s.status === 'pass').length,
    0
  )
  ...
  for (const r of results) {
    ...
    if (r.buildScore) {
      ...
      if (bs.missingFeatures?.length)
        md += `Missing: ${bs.missingFeatures.join(', ')}\n`
    ...
    if (r.verification?.ran) {
      ...
      for (let i = 0; i < r.verification.stories.length; i++) {
        ...
        const icon =
          s.status === 'pass' ? 'PASS' : s.status === 'fail' ? 'FAIL' : '???'
        md += `- **${icon}**: ${story}\n`
      }
...
async function main() {
  ...
  console.log(
    `Scenarios: ${filtered.length}${SCENARIO_FILTER.length ? ` (filtered: ${SCENARIO_FILTER.join(', ')})` : ''}`
  )
  console.log(`Concurrency: ${CONCURRENCY}`)
  console.log(`Timeout: ${TIMEOUT_MS / 1000}s per phase`)
  console.log(
    `Phases: Build${SKIP_VERIFY ? '' : ' → Verify'}${SKIP_DEPLOY ? '' : ' → Deploy'}`
  )
  ...
  console.log(
    `  ${pluginFiles.length} files (${(pluginFiles.reduce((a, f) => a + f.content.length, 0) / 1024).toFixed(0)}KB)\n`
  )
  ...
  await writeFile(
    join(resultsPath, 'results.json'),
    JSON.stringify({ runId, results, totalMs, complete: true }, null, 2)
  )
  ...
  console.log('\n\n=== SUMMARY ===')
  console.log(
    `${'Slug'.padEnd(22)} ${'Build'.padEnd(6)} ${'Skills'.padEnd(6)} ${'Files'.padEnd(6)} ${'Verify'.padEnd(10)} Duration`
  )
  ...
  for (const r of results) {
    ...
    const verify = r.verification
      ...
      : 'skip'
    console.log(
      `${r.slug.padEnd(22)} ${build.padEnd(6)} ${String(r.claimedSkills.length).padEnd(6)} ${String(r.projectFiles.length).padEnd(6)} ${verify.padEnd(10)} ${(r.durationMs / 1000).toFixed(0)}s`
    )
  }
  ...
  for (const r of results) {
    ...
    const extra = [...actual].filter((s) => !expected.has(s))
    console.log(
      `  ${r.slug}: ${hit.length}/${expected.size} expected | +${extra.length} bonus | -${miss.length} missing`
    )
  ...
  if (appsWithUrls.length > 0) {
    ...
    for (const r of appsWithUrls) console.log(`  ${r.slug}: ${r.appUrl}`)
    await writeFile(
      join(resultsPath, 'live-urls.json'),
      JSON.stringify(
        Object.fromEntries(
          appsWithUrls.map((r) => [
            r.slug,
            { url: r.appUrl, sandboxId: r.sandboxId },
          ])
        ),
        null,
        2
