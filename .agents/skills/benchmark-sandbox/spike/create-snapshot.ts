...
async function uploadPluginToSandbox(
  sandbox: InstanceType<typeof Sandbox>
): Promise<void> {
  const pluginFiles = await collectPluginFiles()
  console.log(
    `[snapshot] Uploading ${Object.keys(pluginFiles).length} plugin files to sandbox...`
  )
...
async function createSnapshot(): Promise<SnapshotMetadata> {
  ...
  console.log(
    `[snapshot] Sandbox ${sandbox.sandboxId} created (${elapsed(t1)})`
  )
  ...
  try {
    ...
    if (install.exit !== 0) {
      throw new Error(
        `Claude Code install failed (exit=${install.exit}): ${install.err.slice(0, 300)}`
      )
    }
    ...
    console.log(
      '[snapshot] Uploading local plugin and installing into template dir...'
    )
    ...
    if (!pluginOk) {
      throw new Error(
        `Plugin install failed: ${pluginInstall.out.slice(0, 300)}`
      )
    }
    ...
    if (!hooksCheck.out.includes('EXISTS')) {
      throw new Error(
        `hooks.json not found in template dir after plugin install`
      )
    }
...
async function verifySnapshot(snapshotId: string): Promise<VerifyResult> {
  ...
  console.log(
    `[verify] Sandbox ${sandbox.sandboxId} booted in ${(bootMs / 1000).toFixed(1)}s`
  )
  ...
  try {
    ...
    console.log(
      `[verify] claude --version: ${ver.out} (${claudeVersionOk ? 'OK' : 'FAIL'})`
    )
...
async function main() {
  ...
  console.log(
    `[result] Create time: ${(metadata.createDurationMs / 1000).toFixed(1)}s`
  )
