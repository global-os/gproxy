...
**`X-Gproxy-Navigation` priority patch (fetch-as-navigation):** Cloudflare's bot-management challenge cookie is only issued on a real top-level navigation (`go()`), not a background `fetch()`. To keep using `fetch()` (no cookie persistence across requests, no script execution, no redirect following), the sidecar marks every request with an `X-Gproxy-Navigation: 1` header via `Fetch.continueRequest` (`sidecar/server.mjs`), and `chromium-fork` patches `services/network/url_loader.cc` (the `URLLoader` constructor) to bump `net::RequestPriority` to `net::HIGHEST` when that header is present — so the wire-level HTTP/2 priority reads `u=0` (document navigation) instead of `u=3` (fetch). `services/network/url_loader_util.cc` (`ConfigureUrlRequest`) then strips the marker from the merged outbound headers so it never reaches upstream. **This is a working hypothesis, not verified:** priority is only one navigation signal — `Sec-Fetch-Dest: empty` / `Sec-Fetch-Mode: cors` (still a fetch resource type) and the fetch-style `Accept` remain, so the result is a hybrid fingerprint that may read as *more* suspicious than a clean pass. Point the sidecar at a new build by editing `CHROMIUM_ARTIFACT_SHA` in `petersweb-infra`'s `nixos/linux.nix` and pushing to `main` (pullomatic auto-runs `nixos-rebuild switch`; the container recreates and downloads the new artifact). Caveat: the header-injection and the strip patch must ship together — the marker leaks upstream un-stripped whenever the sidecar runs against a Chromium build older than the strip commit.
...
**Castle SDK obfuscation (reference for re-deobfuscating a new build):** Castle's string literals — property names, the font-fingerprint list, key names, error messages — are never served in plaintext, so `grep`ping a chunk for a string you expect comes up empty. The plaintext only exists in memory, decoded lazily through memoized string-lookup functions (`s(key)` / `s(e,t,n,r)`). `base64` is the *transport container* for the ciphertext inside a text bundle — it is not the obfuscation itself; the decoded bytes are scrambled further. Two schemes we've cracked:

- **X's `castle.umd-*.js` (webpack)**: `atob` → treat bytes as **UTF-16BE code units** (pairs `(b[i]<<8)|b[i+1]`) → **XOR each unit with a rolling LCG keystream** (`state = (40503*state + 13849) & 65535`, per-blob seed) → `String.fromCharCode`. So base64-decoding alone yields gibberish; the XOR keystream is the actual cipher.
- **npm `@castleio/castle-js` 2.8.3–2.8.5 (`castle.browser.js`)**: three layers — **deflate** string table (embedded inflate, the `Uint16Array(16)`-allocating `l()`), then **`atob` base64** per entry, then **per-call factory decoders** (`J`/`W`/`j`/`L`/`F`/`E`/`T`/`O`, memoized, each a slightly different bit-shift transform). ≤2.8.2 uses a different scheme entirely (different string-lookup shape), so decode markers drift between releases.
