...
**Castle.io webpack stub (X-specific):** X's bot-detection SDK (`ondemand.castle.*.js`) is a webpack chunk reported to crash in the cross-origin iframe context. The proxy intercepts it by filename regex, parses the chunk with `acorn` to extract the webpack chunk array name, chunk IDs, and module IDs, then returns a no-op stub: `(self["webpackChunk_twitter_responsive_web"]=...).push([[chunkId],{moduleId:function(){}}])`. This prevents the `ChunkLoadError` that would otherwise cascade and break the login UI — **but also means Castle's real fingerprinting code never runs**, so it never generates the `$castle_token` X's `begin_login` endpoint expects, which is the likely actual cause of the "Please use X.com or official X apps" login error. `CASTLE_DEBUG_REAL=1` serves the real, unstubbed script for testing. See `CASTLE_TOKEN.md` — the crash the stub works around didn't reproduce in repeated local testing with the real script, so this needs more investigation, not just an assumption that the stub is still necessary as-is.
