...
export async function access<T>(
  ...
  if (currentHash !== directory_checksum) {
    throw new Error(
      `Image ${imageId} checksum mismatch — directory has been modified`
    )
  }
...
function addTarDirectory(pack: tar.Pack, entryPath: string): void {
  const entry = new tar.ReadEntry(
    new tar.Header({ path: entryPath, type: 'Directory', size: 0 })
  )
  entry.end()
  pack.add(entry)
}
...
function addTarFile(pack: tar.Pack, entryPath: string, content: Buffer): void {
  const entry = new tar.ReadEntry(
    new tar.Header({ path: entryPath, type: 'File', size: content.length })
  )
  entry.write(content)
  entry.end()
  pack.add(entry)
...
export async function getOrCreateImage(
  ...
  const [existing] = await db
    ...
    .from(image)
    .where(
      and(eq(image.directory_id, directoryId), eq(image.cache_key, cache_key))
    )
    .limit(1)
...
export async function createImage(
  ...
  const compiledFiles = await compileGappTree(
    dirName,
    files,
    opts?.compile
      ? {
